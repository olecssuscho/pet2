...
def payment_request_approve(request:Request,backgroundtask:BackgroundTasks,id:int,user:UserMODEL = Depends(get_current_user),db:Session = Depends(get_db)):
    return payment_request_approve_services(backgroundtask,id,user,db,request)